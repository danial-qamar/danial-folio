...
class AdminPanelProvider extends PanelProvider
{
    public function panel(Panel $panel): Panel
    {
        return $panel
            ...
            ->brandLogo(fn () => $this->getBrandLogoUrl())
            ->brandLogoHeight('2rem')
            ->favicon(fn () => $this->getFaviconUrl())
            ->navigationItems([
            ...
            if (Schema::hasTable('settings')) {
                ...
                if (! empty($design['logo'])) {
                    if (class_exists(\Awcodes\Curator\Models\Media::class)) {
                        $media = \Awcodes\Curator\Models\Media::find($design['logo']);
                        if ($media) {
                            return $media->url ?? asset('storage/' . \Illuminate\Support\Str::after($media->path, 'public/'));
                        }
                    }
                    return asset('storage/' . \Illuminate\Support\Str::after($design['logo'], 'public/'));
                }
            }
        } catch (\Throwable $e) {
            // Fallback if settings table or media model isn't available
        }

        return asset('img/core/logo-app.svg');
    }
        ...
        try {
            if (Schema::hasTable('settings')) {
                ...
                if (! empty($design['favicon'])) {
                    return asset('storage/' . \Illuminate\Support\Str::after($design['favicon'], 'public/'));
                }
            }
        } catch (\Throwable $e) {
            // Fallback if settings table isn't available
        }

        return asset('img/core/favicon.png');
    }
}
