    ...
    <div class="grid gap-4 sm:gap-6 mt-8 sm:mt-12 transition-all duration-300" :class="{
        ...
        @php
            $quickViewPayload = [
                ...
                'title' => $item->title,
                'slug' => $item->slug,
                'image_cover' => $item->project->image_cover ?? null,
                'short_description' => $item->project->short_description ?? null,
                'external_link' => $item->project->external_link ?? null,
                'tags' => $item->project->tags ?? [],
                'category' => [
                    'name' => __(ucfirst($item->project->category->name ?? '')),
                    'icon' => $item->project->category->icon ?? null,
                    'hex_color' => $item->project->category->hex_color ?? null,
                ],
            ];
        @endphp
        <div class="group relative" wire:key='{{ $item->id }}'>
            {{-- Quick View Button --}}
            <button type="button" @click.prevent="$dispatch('open-quick-view', {{ json_encode($quickViewPayload) }})"
                title="{{ __('Quick View') }}"
                class="absolute right-3 top-3 z-40 flex h-7 w-7 items-center justify-center rounded-lg bg-white/90 opacity-0 transition-all duration-300 hover:bg-white group-hover:opacity-100 dark:bg-secondary-900/90 dark:hover:bg-secondary-900 shadow-md">
                ...
                <div class="relative">
                    ...
                    <div class="absolute z-30 left-3 top-3 flex items-center gap-1 rounded-lg px-2 py-0.5 text-[10px] font-medium border transition-colors duration-300"
                        x-show="viewMode !== 'compact'" x-transition :class="{
                            ...
                            '{{ ($item->project->category->color ?? false) ? '' : 'bg-secondary-900' }} text-white bg-opacity-75 border-transparent': !grayscale
                        }" :style="!grayscale ? {
                            backgroundColor: '{{ $item->project->category->hex_color ?? '#111' }}',
                            borderColor: '{{ $item->project->category->hex_color ?? '#111' }}'
                        } : {}">
                        <span wire:ignore>
