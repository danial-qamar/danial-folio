            ...
            <div class="mx-auto max-w-screen-xl px-8 py-8 text-center lg:py-16">
                <figure class="mx-auto max-w-screen-xl">
                    ...
                    <figcaption class="mt-6 flex items-center justify-center gap-2 space-x-2">
                        @if ($picture)
                            @php
                                $pictureUrl = is_numeric($picture) && class_exists(\Awcodes\Curator\Models\Media::class)
                                    ? (\Awcodes\Curator\Models\Media::find($picture)?->url ?? asset('storage/' . $picture))
                                    : asset('storage/' . \Illuminate\Support\Str::after($picture, 'public/'));
                            @endphp
                            <img src="{{ $pictureUrl }}" alt="{{ $author ?? 'Author' }}" class="h-10 w-10 rounded-full object-cover" />
                        @endif
