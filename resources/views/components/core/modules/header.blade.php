...
<title>{{ ($metaTitle && $metaTitle !== 'Danial Qamar | Portfolio' && $metaTitle !== 'Default Title') ? $metaTitle : 'Danial Qamar | Full Stack Laravel Developer' }}</title>
...
<!-- Favicon -->
<link rel="icon" href="{{ $favicon ? asset('storage/' . \Illuminate\Support\Str::after($favicon, 'public/')) : asset('img/core/favicon.png') }}"
    type="image/x-icon">
@if($headerScripts)
<!-- Header Scripts -->
{!! $headerScripts ?? null !!}
