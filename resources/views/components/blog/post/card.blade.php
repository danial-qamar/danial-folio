        ...
        <div
            ...
            <div class="flex-shrink-0">
                <div
                    ...
                    @php
                        $imgCoverUrl = is_numeric($item->post->img_cover) && class_exists(\Awcodes\Curator\Models\Media::class)
                            ? (\Awcodes\Curator\Models\Media::find($item->post->img_cover)?->url ?? asset('storage/' . $item->post->img_cover))
                            : asset('storage/' . \Illuminate\Support\Str::after($item->post->img_cover, 'public/'));
                    @endphp
                    <img class="h-full w-full object-cover" src="{{ $imgCoverUrl }}" alt="{{ $item->title }}" />
                    @else
