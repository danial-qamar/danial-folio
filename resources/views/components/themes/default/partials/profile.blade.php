...
@php
$rawLinkedin = collect($profile->social)->firstWhere('social_network', 'linkedin')['profile_link'] ?? null;
$linkedin = $rawLinkedin ? (preg_match('~^https?://~i', $rawLinkedin) ? $rawLinkedin : 'https://' . $rawLinkedin) : null;
@endphp
    ...
    <section class="mx-auto" id="profile-section-two">
        <div class="flex flex-wrap items-center justify-center gap-4 md:justify-between lg:mt-8 lg:inline-block">
            ...
            <div class="mx-auto py-2 text-center">
                <a href="{{ asset('storage/' . $profile->document) }}" target="_blank">
                    <x-ui.button size="sm" :$is_section_filled_inverted :icon="'download-outline'" :style="'primary'"
